...
const GRADUATION_AMOUNT = 279_900_000_000;
const GRADUATION_SOL_AMOUNT = 100_000_000;
...
const setupMint = async (
  ...
  console.log("Mint ATA:", mintAtaPayer.toString());
  console.log("WSOL Account:", wSolAtaPayer.toString());
  ...
  try {
    console.log("Sending mint initialization transaction...");
  ...
  // Create Mint ATA
  try {
    console.log("Creating Mint ATA...");
    const createMintAtaTx = new Transaction().add(
      createAssociatedTokenAccountInstruction(
        payer.publicKey,
        mintAtaPayer,
        payer.publicKey,
    ...
    await sendAndConfirmTransaction(c, createMintAtaTx, [payer], {
      commitment: "confirmed",
    });
    console.log("Mint ATA created");
  } catch (error) {
    if (!error.message.includes("already in use")) {
      console.error("Error creating Mint ATA:", error);
      throw error;
    }
  }
  ...
  try {
    ...
    console.log("WSOL account created and funded");
  } catch (error) {
  ...
  try {
    ...
    await sendAndConfirmTransaction(c, mintTokensTx, [payer, mintKeypair], {
      commitment: "confirmed",
    });
...
describe("Plasma AMM", async () => {
  const c = new Connection("https://rough-snowy-wish.solana-devnet.quiknode.pro/ad0d64b739b8b308d17e4abb7d3cc4ae9d9cb5b2/", "confirmed");
  //const payer = Keypair.generate();
  const payer = loadWalletKey("/Users/hogyzen12/.config/solana/devDfUnvTUxNCPvW2FgtHsWXJ1YZDv5gUozmyiM8jpM.json");
  ...
  before(async () => {
    console.log("Setting up with wallet:", payer.publicKey.toString());
    // Remove airdrop since wallet is already funded
    [mintPubkey, mintAtaPayer, wSolAtaPayer] = await setupMint(c, payer);
    
    // For traders, we can either:
    // Option 1: Use different funded wallets
    const traderWallets = [
      loadWalletKey("/Users/hogyzen12/.config/solana/devDfUnvTUxNCPvW2FgtHsWXJ1YZDv5gUozmyiM8jpM.json"),
      loadWalletKey("/Users/hogyzen12/.config/solana/yess5Wts82msQzYsRJ7Q2teEcYwoQjsxcLCWL4mMjCm.json"),
    ];
    
    for (const wallet of traderWallets) {
      const [trader, mintAta, wSolAta] = await bootstrapTrader(c, mintPubkey, wallet);
      traders.push([trader, mintAta, wSolAta]);
    }
  });
        ...
        Swap(
          {
            params: {
              ...
              swapType: new ExactIn({
                ...
                minAmountOut: new BN(100_000_000),
              }),
            },
          },
          {
  ...
  describe("Liquidity Provider Operations", () => {
    it("should allow a LP to add more liquidity", async () => {
      // Instead of generating a new keypair and airdropping
      const lpTrader = loadWalletKey("/Users/hogyzen12/.config/solana/yess5Wts82msQzYsRJ7Q2teEcYwoQjsxcLCWL4mMjCm.json");
